...
# jmpout fast check
def jmpout_check(ea, caller_ea_arg = 0):
    global jmpout_collection
    global processed_func
    global processing_func

    func = ida_funcs.get_func(ea)
    func_name = ida_funcs.get_func_name(ea)
    if ea in processed_func or ea in processing_func or func == None or 'sub_' not in func_name:
        return
    
    cfunc = idaapi.decompile(ea)
    processing_func.append(ea)
    call_add_func(ea)
    if "JUMPOUT" in str(cfunc):
        jmpout_collection.append(hex(ea))

    sub_func = get_function_calls(ea)
    for caller_ea, call_ea in sub_func.items():        
        jmpout_check(call_ea, caller_ea)

    processed_func.append(ea)
    if caller_ea_arg == 0:
        if jmpout_collection == []:
            return True
        else:
            return False
...
class jmpout_check_action(idaapi.action_handler_t):
    def __init__(self):
        idaapi.action_handler_t.__init__(self)
    ...
    def activate(self, ctx):
        ...
        if jmpout_check(ea) != True and flag:
            print("[REAI] There func has jmpout")
            print(jmpout_collection)
        else:
            print("[REAI] Jmpout is not exist")     
...
def unregister_actions():
    ...
    idaapi.unregister_action("REAI:jmpout_check") 
...
class ContextMenuHooks(idaapi.UI_Hooks):
    def finish_populating_widget_popup(self, form, popup):
        ...
        if idaapi.get_widget_type(form) == idaapi.BWN_PSEUDOCODE:
            ...
            idaapi.attach_action_to_popup(form, popup, "REAI:jmpout_check", "REAI/")
